...
Requirement is a file path specified in command line with --in /path/to/data/and/error/directory , the code will automatically prompt the user if incorrect

Output name specified, if desired, with --out OUTPUTNAME.root otherwise autogenerated depending on options 
